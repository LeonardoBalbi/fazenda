@php
    ...
    $link = url('/selo_servico/' . base64_encode((string) $selo->id));
    $nomeVisto = strtoupper(optional($selo->alteradoPor)->name ?? '');
    $corBarra = SeloIngressoServico::corBarraDestino($selo->destino);
...
<div class="form-container">
    <div class="label-container">
        <span style="background-color: {{ $corBarra }};">
            <p class="turismo">SELO SERVIÇOS</p>
        ...
        <table>
            <tr>
                ...
                <td>
                    ...
                    <input class="cpf" type="text" id="cnpj-cpf-{{ $selo->id }}" value="{{ $selo->cpf_cnpj }}" readonly>
                </td>
            </tr>
            <tr>
                <td></td>
                ...
                <td>
                    ...
                    <input class="cadastrur" type="text" id="cadastur-{{ $selo->id }}" value="{{ $selo->numero_cadastro_turismo }}" readonly>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <label class="l_ORGANIZADOR" for="organizador-{{ $selo->id }}">ORGANIZADOR(A):</label>
                    <input class="ORGANIZADOR" type="text" id="organizador-{{ $selo->id }}" value="{{ strtoupper((string) ($selo->nome ?? '')) }}" readonly>
                </td>
                <td colspan="2">
                    <label class="l_destino" for="destino-{{ $selo->id }}">DESTINO:</label>
                    <input class="destino" type="text" id="destino-{{ $selo->id }}" value="{{ strtoupper((string) ($selo->destino ?? '')) }}" readonly>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td colspan="2">
                    ...
                    <input class="horario_permanencia_1" type="text" id="horario-permanencia-{{ $selo->id }}" value="{{ $fmtHora($selo->hora_chegada) }}" readonly>
                    <input class="horario_permanencia_2" type="text" value="{{ $fmtHora($selo->hora_saida) }}" readonly>
                </td>
                <td>
    ...
    <span class="assinatura">
        <label class="l_autorizacao" for="numero-guia-{{ $selo->id }}">Nº DA AUTORIZAÇÃO:</label>
        <input class="n_autorizacao" type="text" id="numero-guia-{{ $selo->id }}" value="{{ SeloIngressoServico::mascararAutorizacaoId($selo->id) }}" readonly>
